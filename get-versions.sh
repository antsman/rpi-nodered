...
test -n "$1" && (
  echo NODERED_VERSION=\"$(docker exec -t $1 cat /usr/src/node-red/package.json | grep version | awk '{ print $2 }' | tr -d '",\r')\" | tee $VERSIONS

  echo NODE_VERSION=\"$(docker exec -t $1 node --version | awk -F. '{ print $1 }' | tr -d 'v')\" | tee -a $VERSIONS
